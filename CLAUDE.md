...
It runs inside the editor and exposes 371 tools through:
...
```
All 371 tools (100%) return `{"status": "ok"/"error", ...}` structured dicts as of Phase 21. Zero `None` returns remain in the codebase — MCP callers can read every result directly without parsing log output.
...
tb.register_all_tools()   # ← required — registers all 371 tools
...
**What it proves:** All 371 tools *registered* correctly. The registry loaded, all modules imported, and a set of "safe" tools ran end-to-end without exceptions.
...
**Coverage:** All 371 tools across 21 test sections — materials, bulk ops, patterns, scatter, zones, stamps, actor org, proximity, alignment, signs, post-process, audio, lighting, world state, and more.
...
|---|---|---|
| Tests registration? | ✅ All 371 tools | ✅ |
